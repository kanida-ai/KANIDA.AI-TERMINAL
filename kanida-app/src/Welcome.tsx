...
export function Welcome(){
 const auth=useAuth(),gated=auth.config?.invitation_required!==false;
 ...
 return <View style={{width:'100%',maxWidth:1440,alignSelf:'center',paddingHorizontal:phone?20:42,paddingTop:phone?28:54,paddingBottom:48,gap:wide?42:30}}>
  <View style={{flexDirection:wide?'row':'column',gap:wide?56:28,alignItems:wide?'center':'stretch'}}>
   <View style={{flex:wide?.85:undefined,gap:22}}>
    ...
    <View style={{gap:10,alignItems:wide?'flex-start':'stretch'}}><Button label={auth.user?'Open my workspace':gated?'Enter the private pilot':'Create my account'} icon="arrow-right" onPress={()=>router.push(auth.user?'/':gated?'/signin':'/signup')} style={{minHeight:54,paddingHorizontal:24}}/><T style={{fontSize:14,color:C.muted}}>{gated?'Invitation only':'Registration open'} · your research, plans and broker connection</T></View>
