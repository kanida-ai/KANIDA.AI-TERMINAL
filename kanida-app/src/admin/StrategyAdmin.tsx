...
export function StrategyAdmin(){
 ...
 if(!owner)return <DeskPage><Empty icon="lock" title="Only the owner can manage strategies" detail="The strategy registry decides which strategies every member sees in Discover Strategies. Ask the pilot owner if a strategy should be added or changed." action={<Button label="Go to Discover Strategies" kind="outline" onPress={()=>router.push('/discover' as any)}/>}/></DeskPage>;
 ...
 const askToggleBlock=(b:AdminBlock)=>setConfirm({scope:'block:'+b.key,title:b.enabled?`Disable “${b.title}”?`:`Enable “${b.title}”?`,label:b.enabled?'Disable block':'Enable block',detail:b.enabled?'The block and all its strategy cards disappear from Discover Strategies for every member. Strategies inside it keep their own settings.':`The block appears on Discover Strategies for every member, showing its enabled strategies (${b.strategies.filter(x=>x.enabled).length} of ${b.strategies.length}).`,run:()=>adminApi.updateBlock(b.key,{enabled:!b.enabled})});
 const askToggleStrategy=(b:AdminBlock,x:StrategyDef)=>setConfirm({scope:'block:'+b.key,title:x.enabled?`Disable “${x.name}”?`:`Enable “${x.name}”?`,label:x.enabled?'Disable strategy':'Enable strategy',detail:x.enabled?'It disappears from the Discover Strategies picker and from Falcon for every member. Cards that were using it fall back to the block defaults.':`It appears in the Discover Strategies picker${b.enabled?'':' once the block is enabled'} for every member. Preview it first: rows without a tested exit rule are shown as “not tested”, never as evidence.`,run:()=>adminApi.updateStrategy(x.key,{enabled:!x.enabled})});
 ...
 const header=<View style={[s.between,{flexWrap:'wrap',alignItems:'flex-end'}]}>
  <View style={{gap:6,flexShrink:1}}><T style={s.label}>Owner · Strategy registry</T><T role="heading" aria-level={1} style={[s.title,phone&&{fontSize:24,lineHeight:32}]}>Manage strategies</T><T style={{color:C.muted,maxWidth:720}}>Blocks and strategies shown on Discover Strategies and used by Falcon. Changes apply to every member without a code release.</T></View>
