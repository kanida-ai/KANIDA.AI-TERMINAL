...
export function Badge({label,tone='green',dot=false}:any){const col=tone==='amber'?C.amber:tone==='red'?C.red:tone==='neutral'?C.muted:C.green;return <View style={[s.row,{alignSelf:'flex-start',maxWidth:'100%',gap:5,backgroundColor:tone==='amber'?C.amberBg:tone==='neutral'?C.bg:C.soft,borderRadius:6,paddingHorizontal:8,paddingVertical:4}]}>{dot&&<View style={{width:5,height:5,borderRadius:3,backgroundColor:col}}/>}<T style={{fontSize:10,lineHeight:15,fontFamily:'InterSemi',color:col,flexShrink:1}}>{label}</T></View>;}
