...
import {sb,type Candidate,type DiscoverResult,type Expiry,type Leg} from './api';
...
export function Discover(){
 ...
 async function find(){setBusy(true);setError('');setPick([]);
  try{setRes(await sb.discover({underlying:u,expiry:e,view,target:target||undefined,low:low||undefined,high:high||undefined,max_loss:maxLoss||undefined,budget:budget||undefined,hedged_only:hedged,lots}));}
 ...
 return <View style={{padding:wide?24:14,gap:16,maxWidth:1200,width:'100%',alignSelf:'center'}}>
  ...
  <View style={{backgroundColor:C.paper,borderWidth:1,borderColor:C.line,borderRadius:14,padding:16,gap:14}}>
   ...
   <T style={{fontSize:12,color:C.amber}}>{`Stored reading · ${istStamp(asOf)} · spot ${num(spot,2)} · not live`}</T>
   ...
   <View style={[s.row,{flexWrap:'wrap',gap:12}]}>
    ...
    {field('Capital budget (₹, optional)',budget,setBudget,'e.g. 20000')}
   ...
   <View style={{flexDirection:wide?'row':'column',flexWrap:'wrap',gap:12}}>
    {res.candidates.map((c,i)=><View key={c.template} style={{flexBasis:wide?'31%':'auto',flexGrow:1,backgroundColor:C.paper,borderWidth:1,borderColor:pick.includes(c.template)?C.green:C.line,borderRadius:14,padding:14,gap:8}}>
     ...
     <View style={[s.row,{flexWrap:'wrap',gap:12}]}>
      ...
      <Kv k="Breakeven" v={c.breakevens.map(b=>num(b,0)).join(' · ')}/><Kv k="At risk" v={inr(c.capital_at_risk)}/><Kv k="POP (model)" v={c.pop!=null?`${c.pop}%`:'—'}/>
     </View>
     <View style={[s.row,{gap:8,flexWrap:'wrap'}]}>
      <Checkbox checked={pick.includes(c.template)} onChange={on=>setPick(p=>on?(p.length>=3?p:[...p,c.template]):p.filter(x=>x!==c.template))} label="Compare"/>
      <View style={{flex:1}}/><Button label="Use as draft" icon="arrow-right" onPress={()=>use(c)}/></View>
    </View>)}
   </View>
