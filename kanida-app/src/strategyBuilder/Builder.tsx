...
import {sb,type Analysis,type Body,type Chain,type Detail,type Expiry,type Kind,type Leg,type PaperRun,type Side,type Template} from './api';
import {ChainDrawer} from './ChainDrawer';
import {PayoffChart} from './PayoffChart';
import {addDays,dayMonth,inr,isWeekend,istStamp,num,signed,strikeText,weekday} from './format';
...
export function Builder({id,openTemplate=false}:{id:string;openTemplate?:boolean}){
 ...
 const loadRuns=useCallback(async()=>{try{const r=await sb.paperList();setRuns(r.runs.filter(x=>x.strategy_id===id));}catch{}},[id]);
 ...
 useEffect(()=>{if(!body?.underlying||!body.expiry){setChain(null);return;}const c=new AbortController();
  ...
  return()=>c.abort();},[body?.underlying,body?.expiry]);
 ...
 useEffect(()=>{if(!body)return;clearTimeout(anTimer.current);setPending(true);
  ...
  return()=>clearTimeout(anTimer.current);},[body]);
 ...
 function toggleFromChain(strike:number,kind:Kind,side:Side){
  edit(b=>{const hit=b.legs.find(l=>l.strike===strike&&l.type===kind);
   ...
   return {...b,template:null,legs:[...b.legs,{id:uid(),type:kind,side,strike,lots:1,expiry:b.expiry,price_basis:'ltp',price:null,include:true}]};});
 }
 ...
 async function applyTemplate(t:Template,param:number|null){
  ...
  try{const r=await sb.resolve(t.key,body.underlying,body.expiry,param);
   const legs:Leg[]=r.legs.map((l:any)=>({id:uid(),type:l.type,side:l.side,strike:l.strike,lots:l.lots,expiry:l.expiry,price_basis:'ltp',price:null,include:true}));
 ...
 return <View style={{padding:wide?22:14,gap:16,maxWidth:1500,width:'100%',alignSelf:'center'}}>
  ...
  <View style={[s.between,{flexWrap:'wrap',gap:10}]}>
   ...
   <View style={[s.row,{flexWrap:'wrap',gap:8,width:wide?undefined:'100%'}]}>
    ...
    <Button label="Paper trade" icon="play" onPress={()=>setPaperOpen(true)} disabled={!body.legs.length||!a||a.status==='invalid'}/>
   </View>
  </View>
  <DataBanner reading={reading} live={!!chain?.quality.live}/>
     ...
     body.legs.map(l=>{const row=a?.legs?.find(r=>r.id===l.id);const ltp=priceOf(l.strike,l.type);
      return <View key={l.id} style={{borderTopWidth:1,borderColor:C.line,paddingTop:10,gap:8,opacity:l.include?1:.5}}>
       ...
       <View style={[s.row,{flexWrap:'wrap',gap:14}]}>
        <PriceField leg={l} ltp={ltp} onManual={(v)=>setLeg(l.id,{price_basis:'manual',price:v})} onLtp={()=>setLeg(l.id,{price_basis:'ltp',price:null})}/>
    ...
    <View style={panel}>
     <View style={[s.row,{flexWrap:'wrap',gap:6}]}>
      {(['pnl','greeks','table','snapshots','paper','activity'] as Tab[]).map(t=><Chip key={t} active={tab===t} onPress={()=>setTab(t)}
       label={{pnl:'P&L by leg',greeks:'Greeks',table:'Payoff table',snapshots:`Snapshots (${detail.snapshots.length})`,paper:`Paper (${runs.length})`,activity:'Activity'}[t]}/>)}
     </View>
     ...
     {tab==='activity'&&<View style={{gap:6}}>{detail.activity.map((x,i)=><View key={i} style={[s.between,{borderTopWidth:1,borderColor:C.line,paddingTop:6}]}><T style={{fontSize:12,flex:1}}>{x.detail}</T><T style={{fontSize:11,color:C.muted}}>{new Date(x.created_at*1000).toLocaleString()}</T></View>)}</View>}
    </View>
   </View>
  </View>
...
function DataBanner({reading,live}:{reading?:string;live:boolean}){
 return <View style={[s.row,{flexWrap:'wrap',gap:8,backgroundColor:live?C.soft:C.amberBg,borderRadius:10,paddingHorizontal:12,paddingVertical:8}]}>
  ...
  <T style={{fontSize:12,color:live?C.green:C.amber,flex:1}}>{live?`Live · ${istStamp(reading)}`:`Stored reading · ${istStamp(reading)} · not live. Prices are last traded (this store has no bid/ask). Research and paper only - nothing here sends an order.`}</T>
 </View>;
}
...
function PriceField({leg,ltp,onManual,onLtp}:{leg:Leg;ltp:number|null;onManual:(v:number)=>void;onLtp:()=>void}){
 ...
 return <View style={[s.row,{gap:6}]}>
  ...
  <TextInput value={leg.price_basis==='manual'?text:(ltp!=null?String(ltp):'')} onChangeText={setText} onFocus={()=>{if(leg.price_basis==='ltp')setText(ltp!=null?String(ltp):'');}}
  ...
  {leg.price_basis==='manual'?<Pressable accessibilityRole="button" onPress={onLtp}><T style={{fontSize:11,color:C.amber}}>manual · use LTP</T></Pressable>:<T style={{fontSize:11,color:C.muted}}>LTP</T>}
 </View>;
}
...
function RiskStrip({a,dim}:{a:Analysis|null;dim:boolean}){
 ...
 const items=[
  ...
  ['Margin','Needs broker',C.muted,'Exchange margin needs a connected broker'],
 ] as const;
...
function PaperList({runs,onClose,onOpen}:{runs:PaperRun[];onClose:(id:string)=>void;onOpen:()=>void}){
 ...
 return <View style={{gap:10}}>{runs.map(r=><View key={r.id} style={{borderTopWidth:1,borderColor:C.line,paddingTop:8,gap:4}}>
  <View style={s.between}><View style={[s.row,{gap:8}]}><Badge label={r.status==='open'?'PAPER · OPEN':'PAPER · CLOSED'} tone={r.status==='open'?'green':'neutral'}/><T style={{fontSize:12,color:C.muted}}>{`Snapshot #${r.revision.n} · opened ${istStamp(r.opened_reading)}`}</T></View>
   {r.status==='open'&&<Button label="Close at stored reading" kind="outline" onPress={()=>onClose(r.id)}/>}</View>
...
function TemplateSheet({visible,onClose,onPick,replacing}:{visible:boolean;onClose:()=>void;onPick:(t:Template,p:number|null)=>void;replacing:number}){
 ...
 return <Sheet visible={visible} onClose={onClose} wide title="Choose a template" subtitle={replacing?`Using a template replaces the ${replacing} current leg${replacing>1?'s':''} (undo with a snapshot restore).`:'A template is a recipe; it becomes exact contracts from the stored chain, all editable.'}>
  {!data?<Loading/>:<>
