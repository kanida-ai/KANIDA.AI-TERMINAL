...
function PaperRuns(){
 ...
 return <View style={{padding:wide?24:14,gap:16,maxWidth:1100,width:'100%',alignSelf:'center'}}>
  ...
  <View style={{gap:4}}><T style={{fontFamily:'ManropeBold',fontSize:22}}>Paper runs</T>
   <T style={{fontSize:13,color:C.muted}}>Simulated ledgers only - no order was ever sent to a broker. Two modes, shown on every run: <T style={{fontFamily:'InterSemi',color:C.ink}}>Live-quote paper</T> fills limit orders against live bid/ask, only up to the size on show; <T style={{fontFamily:'InterSemi',color:C.ink}}>Stored-price practice</T> fills at a stored last price moved against you by slippage. Both include estimated charges.</T></View>
  ...
  {cap&&<T style={{fontSize:12,color:C.muted}}>{`Live-quote paper capital (deployments only) ${inr(cap.capital)} · margin blocked ${inr(cap.blocked)} · available ${inr(cap.available)}. Stored-price practice runs have no capital account.`}</T>}
  {deps.length>0&&<View style={{gap:10}}><T style={{fontFamily:'InterSemi'}}>Live-quote paper</T>
  ...
  <T style={{fontFamily:'InterSemi'}}>Stored-price practice</T>
  {runs===null?(error?null:<Loading/>):!runs.length?<Empty icon="play" title="No paper runs yet" detail="Open a strategy and choose Paper trade to record simulated fills against a frozen snapshot."/>:
   runs.map(r=><View key={r.id} style={{backgroundColor:C.paper,borderWidth:1,borderColor:C.line,borderRadius:14,padding:14,gap:8}}>
    <View style={[s.between,{flexWrap:'wrap'}]}>
     <View style={[s.row,{gap:8,flexWrap:'wrap'}]}><Badge label={`PRACTICE · ${r.status==='open'?'OPEN':'CLOSED'}`} tone={r.status==='open'?'green':'neutral'}/>
    ...
    {/* P08: when you acted vs which prices were used - never one date standing for both */}
    <T style={{fontSize:12,color:C.muted}}>{`Opened ${istStamp((r as any).opened_action_at||r.opened_reading)} using prices from ${istStamp(r.opened_reading)}${r.closed_reading?` · closed ${istStamp((r as any).closed_action_at||r.closed_reading)} using prices from ${istStamp(r.closed_reading)}`:` · marked at ${istStamp(r.as_of)}`}`}</T>
    {!!(r as any).eligible&&<T style={{fontSize:11,color:C.muted}}>{['monitor','alerts','adjust'].filter(k=>!(r as any).eligible[k]?.ok).map(k=>(r as any).eligible[k].reason).filter((x:string,i:number,a:string[])=>a.indexOf(x)===i).join(' ')}</T>}
