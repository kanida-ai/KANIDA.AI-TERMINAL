...
function Backtest({strategyId,preset}:{strategyId:string;preset:{template:string;param?:number;underlying?:string;lots?:number;structure?:string}}){
 ...
 return <View style={{gap:14}}>
  <View style={{backgroundColor:C.paper,borderWidth:1,borderColor:C.line,borderRadius:14,padding:16,gap:12}}>
   ...
   <View style={[s.row,{flexWrap:'wrap',gap:6}]}>{tpls.filter(x=>!(x as any).multi_expiry).map(x=><Chip key={x.key} label={x.name+(x.risk==='unhedged'?' ⚠':'')} active={tpl===x.key} onPress={()=>setTpl(x.key)}/>)}</View>
   {/* P13: unsupported structures are stated before Run, never queued and rejected afterwards */}
   {tpls.some(x=>(x as any).multi_expiry)&&<T style={{fontSize:11,color:C.muted}}>{`Not testable in the Lab yet: ${tpls.filter(x=>(x as any).multi_expiry).map(x=>x.name).join(', ')} - rule tests value one expiry at a time. Use the builder's model and replay for these.`}</T>}
...
function ReplayView({strategyId}:{strategyId:string}){
 ...
 return <View style={{backgroundColor:C.paper,borderWidth:1,borderColor:C.line,borderRadius:14,padding:16,gap:12}}>
  ...
  {!r?<Loading/>:<>
   ...
   {r.coverage_detail&&<View style={{gap:2}}><T style={{fontSize:11,color:C.muted}}>{`Coverage: ${r.coverage_detail.common_bars} bars with a price for every leg - ${(r.coverage_detail as any).denominator==='expected_session_bars'?`${Math.round(r.coverage_detail.share*100)}% of the ${(r.coverage_detail as any).expected_bars} exchange-session bars in the window`:`${Math.round(r.coverage_detail.share*100)}% of the best-covered leg (the expected session count was not available)`}; minimum ${Math.round(r.coverage_detail.minimum*100)}%${(r.coverage_detail as any).longest_intraday_gap_minutes?` · longest gap ${(r.coverage_detail as any).longest_intraday_gap_minutes} min`:''}. Gross traded prices - not a net strategy test.`}</T>
