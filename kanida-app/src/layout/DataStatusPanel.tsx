...
import {FNO_SEPARATE_TEXT,fnoCaptureView,useFnoCapture,type FnoCaptureView} from './fnoCapture';
...
export type DataStatusPanelProps={view:DataStatusView;
 /** F&O CAPTURE, THE OTHER FEED. Everything else in this panel describes prices and patterns; this block
  *  describes what was measured in the F&O store, and it is present only when that is a caveat. The two are
  *  never merged into one verdict: on 18 Sep 2026 the cash feed was healthy all afternoon while F&O capture
  *  was dark, and a single combined chip would have had to be wrong about one of them. */
 fno?:FnoCaptureView|null};
export function DataStatusPanel({view,fno}:DataStatusPanelProps){
 ...
 return <ScrollView style={{maxHeight:460}} contentContainerStyle={{paddingHorizontal:14,paddingVertical:10,gap:4}}>
  ...
  {!!fno&&<>
   <View style={{height:1,backgroundColor:C.line,marginVertical:6}}/>
   <View role="status" accessibilityLabel={fno.a11y} style={{gap:4,padding:9,borderRadius:9,
    backgroundColor:C.amberBg,borderWidth:1,borderColor:C.amber+'55'}}>
    <View style={[s.row,{gap:8,alignItems:'flex-start'}]}>
     <Icon name="alert-triangle" size={14} color={C.amber}/>
     <T style={{flex:1,fontSize:12,lineHeight:18,fontFamily:'InterSemi',color:C.amber}}>{fno.headline}</T>
    </View>
    {/* the SERVER's own sentence, not a paraphrase of it */}
    <T style={{fontSize:11,lineHeight:17,color:C.amber}}>{fno.detail}</T>
    <T style={{fontSize:10,lineHeight:15,color:C.muted}}>{FNO_SEPARATE_TEXT}</T>
   </View>
  </>}
 </ScrollView>;
}
...
export function DataStatusPopover({open,onClose,anchor,status,dataEnd,ageDays,staleLimit,placement='bottom-end',routeKey,cache,error}:DataStatusPopoverProps){
 ...
 // The same one answer the pill's chip is built on - one request, two surfaces, and no way for the chip and
 // the panel it opens to disagree about the F&O store.
 ...
 return <Popover open={open} onClose={onClose} anchor={anchor} placement={placement} label="Data status" width={330} routeKey={routeKey}>
  <View ref={first} {...webOnly({tabIndex:-1})} style={{flexShrink:1}}><DataStatusPanel view={view} fno={fno}/></View>
 </Popover>;
}
