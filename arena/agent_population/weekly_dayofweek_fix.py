"""Test the day-of-week weekly fix: recompute the 4 weekly features over a FULL trailing 5-trading-day window
(day-of-week independent, always 'mature') instead of calendar-week-to-date (1 day Mon .. 5 days Fri).
Leak-free (trailing windows only). Re-rank Jan-2025; measure Bucket-2 (16-30) promotion + top-15 retention
vs baseline (calendar week-to-date). Read-only."""
...
def build_weekly(mode):
    o2=oh.copy(); dt=pd.to_datetime(o2.trade_date); o2["wk"]=dt.dt.isocalendar().year.astype(int)*100+dt.dt.isocalendar().week.astype(int)
    o2["dow"]=dt.dt.dayofweek+1  # 1=Mon..5=Fri
    rec=[]
    for s,g in o2.groupby("symbol",sort=False):
        g=g.sort_values("trade_date").reset_index(drop=True); c=g.close.values.astype(float); h=g.high.values.astype(float); l=g.low.values.astype(float)
        dow=g.dow.values.astype(float); dpos=np.clip(dow,1,5)
        # calendar week-to-date (baseline components)
        hi=g.groupby("wk").high.cummax().values; lo=g.groupby("wk").low.cummin().values
        wb=g.groupby("wk").agg(wc=("close","last"),wh=("high","max")).reset_index(); wb["sm"]=wb.wc.rolling(20).mean().shift(1); wb["ph"]=wb.wh.rolling(20).max().shift(1)
        sm=g.wk.map(dict(zip(wb.wk,wb.sm))).values; ph=g.wk.map(dict(zip(wb.wk,wb.ph))).values
        wcl=np.where(hi>lo,(c-lo)/(hi-lo),np.nan); wrp=np.where(c>0,(hi-lo)/c*100,np.nan)
        wsm=np.where((sm==sm)&(sm>0),(c/sm-1)*100,np.nan); wbo=np.where(ph==ph,(c>ph).astype(float),np.nan)
        # trailing-5d
        t_hi=pd.Series(h).rolling(5,min_periods=2).max().values; t_lo=pd.Series(l).rolling(5,min_periods=2).min().values
        t_wcl=np.where(t_hi>t_lo,(c-t_lo)/(t_hi-t_lo),np.nan); t_wrp=np.where(c>0,(t_hi-t_lo)/c*100,np.nan)
        if mode=="calweek": pass
        elif mode=="trail5":
            sma100=pd.Series(c).rolling(100,min_periods=40).mean().values; ph2=pd.Series(h).rolling(100,min_periods=40).max().shift(1).values
            wcl=t_wcl; wrp=t_wrp; wsm=np.where((sma100==sma100)&(sma100>0),(c/sma100-1)*100,np.nan); wbo=np.where(ph2==ph2,(c>ph2).astype(float),np.nan)
        elif mode=="trail5range":  # ONLY range = trailing-5d, rest = calendar
            wrp=t_wrp
        elif mode=="projrange":    # range projected to full week: wtd_range * 5/dow (capped)
            wrp=np.where(c>0, wrp*np.minimum(5.0/dpos,5.0), wrp)
        elif mode=="proj_both":    # range projected + close_loc = trailing-5d (more mature) on early days only
            wrp=np.where(c>0, wrp*np.minimum(5.0/dpos,5.0), wrp)
            wcl=np.where(dpos<=3, t_wcl, wcl)
        rec.append(pd.DataFrame(dict(symbol=s,trade_date=g.trade_date.values,
            weekly_close_loc=wcl,weekly_range_pct=wrp,weekly_close_vs_sma20=wsm,weekly_breakout_20w=wbo)))
    return feat.drop(columns=WEEKLY).merge(pd.concat(rec,ignore_index=True),on=["symbol","trade_date"],how="left")
...
results={}
for mode in ["calweek","trail5","trail5range","projrange","proj_both"]:
    FCpit=build_weekly(mode); rk={td:(ranks(FCpit,sdmap[td]) if sdmap[td] else {}) for td in OPJAN}
    results[mode]=rk
base=results["calweek"]
...
def stats(rk):
    top15=sum(1 for td,s in allpick if sdmap[td] and rk[td].get(s) and rk[td][s]<=15)
    ...
    listed=sum(1 for td,s in allpick if sdmap[td] and rk[td].get(s))
    return top15,prom,kept,listed
print(f"  {'weekly mode':<28}{'net top-15':>12}{'bkt2->15':>10}{'top15 kept':>12}{'ranked(top100)':>15}")
for mode,lab in [("calweek","calendar week-to-date (base)"),("trail5","trailing-5d all"),("trail5range","range=trailing5d only"),("projrange","range x 5/dayofweek"),("proj_both","range-proj + early close_loc")]:
    t,p,k,l=stats(results[mode])
    print(f"  {lab:<28}{t:>12}{p:>7}/{len(bbucket2):<2}{k:>7}/{len(bmatched):<3}{l:>15}")
print(f"\n  baseline top-15={len(bmatched)}, bucket-2={len(bbucket2)}. Goal: trailing-5d raises net top-15 (promote bkt2, keep the 57).")
