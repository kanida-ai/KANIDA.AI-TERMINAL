...
# merge sector / relative-strength / rsi from falcon_features (daily, leak-free trailing windows)
con2 = sqlite3.connect("file:" + UDB.replace("\\", "/") + "?mode=ro", uri=True)
ff = pd.read_sql_query("SELECT symbol,trade_date,rs_sector_60d,rs_market_60d,rsi_14 FROM falcon_features "
                       "WHERE trade_date>='2024-06-01' AND trade_date<='2025-06-30'", con2); con2.close()
P = P.merge(ff, on=["symbol", "trade_date"], how="left")
EXTRA = ["rs_sector_60d", "rs_market_60d", "rsi_14"]
print(f"feature panel: {len(P):,} rows (+{len(EXTRA)} sector/RS/rsi)")
...
def recall_month(train, test):
    ...
    for sd, g in test.groupby("sd"):
        ...
        w = g[g.y == 1].symbol; tot += len(w)
        for k in hit:
            top = set(order[:k]); hit[k] += sum(s in top for s in w)
        bask.append(np.nanmean([RET_OC.get((s, g.td.iloc[0]), np.nan) for s in order[:15]]))
    R = lambda k: (hit[k] / tot * 100 if tot else 0)
    return R(5), R(15), R(30), tot, float(np.nanmean(bask)) * 5, clf, test

months = sorted(S.mo.unique())
print("\n============ WALK-FORWARD v2 (train prior -> test next; +sector/RS, recall@5/15/30) ============")
print(f"{'test month':<12}{'train N':>9}{'winners':>9}{'r@5':>6}{'r@15':>7}{'r@30':>7}{'top15 5x':>10}")
agg = {5: 0, 15: 0, 30: 0}; alltot = 0; lastclf = None; deliver = []
for i in range(2, len(months)):
    tr = S[S.mo.isin(months[:i])]; te = S[S.mo == months[i]]
    if len(te) == 0 or te.y.sum() == 0: continue
    r5, r15, r30, tot, bask, lastclf, scored = recall_month(tr, te)
    agg[5] += r5 / 100 * tot; agg[15] += r15 / 100 * tot; agg[30] += r30 / 100 * tot; alltot += tot
    print(f"  {months[i]:<10}{int(tr.y.sum()):>9}{tot:>9}{r5:>5.0f}%{r15:>6.0f}%{r30:>6.0f}%{bask:>+9.2f}%")
    for sd, g in scored.groupby("sd"):
        g = g.sort_values("p", ascending=False).head(30)
        for rnk, (_, rr) in enumerate(g.iterrows(), 1):
            deliver.append(dict(signal_date=sd, trade_date=rr.td, rank=rnk, symbol=rr.symbol,
                                model_score=round(rr.p, 4), my_winner=("YES" if rr.y == 1 else ""),
                                ret_oc_next=round(RET_OC.get((rr.symbol, rr.td), np.nan), 2)))
OUTX = os.path.join(os.path.expanduser("~"), "Downloads", "OPERATOR_SCREENER_SHORTLIST.xlsx")
pd.DataFrame(deliver).to_excel(OUTX, index=False)
print(f"  daily ranked top-30 shortlist -> {OUTX}")
print(f"\n  overall forward recall  @5 {agg[5]/alltot*100:.0f}%  @15 {agg[15]/alltot*100:.0f}%  @30 {agg[30]/alltot*100:.0f}%  (over {alltot} winners)")
# feature importance (impurity via a quick permutation on the full panel)
from sklearn.inspection import permutation_importance
te = S[S.mo == months[-1]]
pi = permutation_importance(lastclf, te[RC].values, te.y.values, n_repeats=3, random_state=0, n_jobs=1)
imp = pd.Series(pi.importances_mean, index=XC).sort_values(ascending=False)
print("\n  top 15 feature drivers (what the screener leans on):")
for f, v in imp.head(15).items(): print(f"    {f:<14}{v:+.5f}")
