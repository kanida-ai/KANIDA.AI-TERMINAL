...
from .hypotheses import VariantEvidence, ReplayStats
...
def worth_testing_gates(ev: VariantEvidence, *, c: Constitution, xcfg: ExperimentConfig, n_trials: int,
    ...
    g.add("history_sample", w.n >= xcfg.min_n_history, float(w.n), float(xcfg.min_n_history),
          "cases on the whole sealed history; below the bar the number is labelled, not tested")
    g.add("history_expectancy_net", _ok(w.expectancy_net, _gt(c, "min_expectancy_pct_net")), _v(w.expectancy_net),
          _gt(c, "min_expectancy_pct_net"), "expectancy per trade net of costs and slippage on the whole sealed history")
    ...
    g.add("history_placebo", _ok(w.placebo_p, alpha, above=False), _v(w.placebo_p), alpha,
          f"day-blocked permutation p-value ({w.placebo_draws} draws): a screen, not a significance claim")
    g.add("trailing_sample", t.n >= xcfg.min_n_trailing, float(t.n), float(xcfg.min_n_trailing),
          f"cases on the trailing validation window (after {xcfg.discovery_end})")
    ...
    g.add("family_wise_significance", _ok(w.placebo_p, adjusted, above=False), _v(w.placebo_p), adjusted,
          f"the bar the best of {n_trials} counted trials must clear to be called significant; advisory here", fatal=False)
    ...
    g.add("history_cluster_significance", _ok(w.cluster_t, _z(c)), _v(w.cluster_t), _z(c),
          f"t on {w.signal_days} independent signal days, not on {w.n} trades; advisory here", fatal=False)
    return g
...
def graduation_gates(fr: ForwardRecord, *, incumbent: Optional[BookRun], direction: str, horizon: int,
    ...
    g.add("constitution_signed", signed, None, None,
          ("Constitution is signed" if signed else
           f"Constitution {c.version} is an UNSIGNED draft ({c.approved_by}); nothing may be promoted until a human signs it"))
    ...
    g.add("oos_placebo", _ok(fr.placebo_p, alpha, above=False), _v(fr.placebo_p), alpha,
          f"day-blocked permutation p-value over the forward window ({fr.placebo_draws} draws)")
    g.add("oos_cluster_significance", _ok(fr.cluster_t, _z(c)), _v(fr.cluster_t), _z(c),
          "t on independent forward signal days")
    ...
    if incumbent is None or incumbent.total_return_pct is None:
        ...
        g.add("no_worse_drawdown_than_incumbent", fr.max_drawdown_pct <= inc_dd, fr.max_drawdown_pct, inc_dd,
              "worst drawdown of that compounded curve vs the incumbent's")
...
def gate_rows(g: GateSet) -> list[dict]:
    return [{"name": x.name, "passed": bool(x.passed),
             ...
             "statement": x.statement, "fatal": bool(x.fatal)} for x in g.gates]
