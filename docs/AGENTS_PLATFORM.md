...
| **Chart Agent — strategy-replay SHORT mirror (WAVE A)** (`strategy.py`: direction-aware `_replay_core`; short entry, stop ABOVE / target BELOW, TRAIL off the running trough, INVALIDATION on close reclaiming the level; inverted MFE/MAE/drawdown; short baseline; new frozen ids `S-trendline-v1` / `S-trendline-short-v1`, `S-horiz-v1` kept) | **BUILT (this branch)** — the LONG path is **BYTE-IDENTICAL** (all 15 prior tests + TITAN unchanged); short STOP/TARGET/TRAIL/INVALIDATION/HORIZON all fire; pattern-vs-strategy separation holds short too |
| **Chart Agent — direction-aware evidence** (`evidence.py`: `pattern_evidence`/`_baseline_edges`/`decide` gain `direction`; short negates the forward return and swaps MFE/MAE; decide picks the same-direction baseline; `agent.decide` threads occurrence direction + direction-appropriate policy + direction-aware Intent thesis, still `mode="paper"`) | **BUILT (this branch)** — long branch byte-identical; two outcome families stay separate both directions |
...
| **Chart Agent — SHARED SLOPED-GEOMETRY ENGINE (WAVE A)** (`patterns/_geometry.py`: `pivot_highs/lows`, two-anchor+containment `fit_line` (deterministic: most-touches → smallest SSR → smallest a → smallest b; r2 reported-only, never the selector), `fit_as_of`, ordered `classify_taxonomy`, `sloped_live_stage`, `sloped_historical_events`, shared `SlopedDetector` base) | **BUILT (this branch)** — pure functions over an already-sliced array; **PIT by construction** (pivots ≤ k−L; containment scan `close[min(anchor):k]` EXCLUSIVE of k; breakout on close[k−1]→close[k]; apex_x is a projected intersection that reads no bar). Mirrors `horizontal_trendline.classify` bar-for-bar with a sloped line. Per-symbol fit cache (df.attrs) so all 7 detectors share one fit. **Windowed==full-history detect verified both directions, 0 mismatches / 0 future-leaks** over all sloped setups |
| **Chart Agent — patterns 1–7: ascending/descending/symmetrical triangle · rising/falling wedge · rectangle · channel** (thin modules, each `status="built"`, set `breakout_side`/`direction`, self-register; the old generic `triangle.py` skeleton RETIRED; `channel.py` skeleton REPLACED) | **BUILT (this branch)** — on R&D 2022-08-30 the full-universe screen returns **106 setups** across all built patterns (ascending 9, falling_wedge 8, symmetrical 5, rectangle 3, channel 3, rising_wedge 2, + horizontal 76), long **and** short. Ascending/descending/symmetrical + wedges carry an apex/expiry gate; rectangle/channel do not. 10 WAVE A tests (per-pattern synthetic · taxonomy one-label · flat_eps boundary flip · apex expiry · future-bar-ignored · real windowed==full · resolved-only events · short replay · short separation · direction-aware baseline) — all pass |
| **Chart Agent — governed sloped knobs** (`flat_eps=5e-4`, `parallel_eps=0.15`, `apex_max_frac=0.75`, `min_pattern_bars=15`, `breakdown_vol_mult=vol_mult`) | **BUILT but SPEC-until-OOS** — frozen defaults in `GEOM_PARAMS`; calibrate only by walk-forward + discovery/OOS split in Loop-4, never on the deciding sample |
| **Chart Agent — pattern 8: Cup & Handle (WAVE B)** (`patterns/cup_and_handle.py`, `status="built"`, BULLISH/long only) | **BUILT-basic (this branch)** — DETERMINISTIC + point-in-time: two similar-height rims (`pivot_highs`, index ≤ k−L, within `rim_tol`; rim = the lower), rounded U base via least-squares quadratic on the between-rim closes (require curvature>0, `R²≥cup_r2_min`, a **PARABOLA-vs-TENT discriminator** `quad_ssr/tent_ssr < tent_ratio_max` — the tent is the best symmetric V centred on the base — that genuinely rejects wide symmetric V bottoms (R²≈0.94) and blocky staircases (R²≈0.89) which a bare R² floor lets through, depth in `[cup_depth_min,cup_depth_max]`, argmin near middle `symmetry_frac`), a REQUIRED shallow handle (`handle_depth_min..handle_depth_max`×cup_depth, `handle_max_bars`, low ≥ cup-mid, no early rim break), then the bullish rim breakout (close[k−1]≤rim·(1+buffer)<close[k] on volume; level = rim frozen scalar). Reuses the LONG strategy-replay + pattern-forward unchanged. MEASURED on R&D (post tent-gate honesty fix): **4 cups on 2022-08-30, 0 on 2021-11-15, 0 on 2023-07-03** (the earlier 11/7 counts included symmetric-V / staircase shapes now correctly rejected — the drop is expected and correct); full-universe precompute ~16–30 s. Windowed==full-history detect, **0 mismatches / 0 future-leaks** over the real cups. 7 tests (registration · synthetic BREAKOUT · V/no-handle×2/too-deep/too-shallow rejected · **tent-discriminator rejects V/staircase, true-U still fires** · future-bar-ignored · real windowed==full · resolved-only events). **Small-N → honest WATCH**, nothing presented as validated. (The FAILED live stage is gated to down-close days — not an exhaustive failure catalogue.) |
| **Chart Agent — Cup & Handle governed knobs** (`rim_tol=0.05`, `cup_r2_min=0.60`, `tent_ratio_max=0.50`, `cup_depth_min=0.08`, `cup_depth_max=0.50`, `handle_depth_min=0.01`, `handle_depth_max=0.33`, `handle_max_bars=20`, `symmetry_frac=0.25`, `min_cup_bars=20`, `max_cup_bars=130`) + **SPEC** rim-symmetry robustness & volume-profile base confirmation | **BUILT but SPEC-until-OOS** — frozen defaults in `CUP_PARAMS`; the argmin-based symmetry test and quadratic-R² roundedness are BASIC heuristics, and base-volume-profile confirmation is not built; calibrate/extend only in Loop-4 |
